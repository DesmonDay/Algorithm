...
for i = 1:200
    ...
    if i<=20
        first1 = norm(S-M*U','fro')^2;
        second1 = alpha*norm(H-U*C','fro')^2;
        third1 = -beta*trace(H'*(B1-B2)*H);
        constraint1 = lambda*norm(H'*H-I,'fro')^2;
        L_temp = first1 + second1 + third1 + constraint1;
        plot(i,L_temp,'r*');
        hold on;
    end
end
